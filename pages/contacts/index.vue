<template>
  <div class="d-flex flex-column flex-grow-1">
    <div class="d-flex align-center py-1">
      <div>
        <div class="display-1">{{ $t('menu.contacts') }}</div>
        <breadcrumbs />
      </div>
      <v-spacer />
      <v-btn color="primary" :to="{ name: 'contacts-create' }">{{ $t('contact.action.createContact') }}</v-btn>
    </div>
    <list-filter v-model="params" @filter="filterContact" @reset="resetFilter" />
    <v-card class="mt-3">
      <v-row class="align-center pa-2" dense>
        ...
        <v-col cols="6" class="d-flex align-center text-right">
          <app-text-field
            v-model="search"
            :placeholder="$t('contact.message.searchHint')"
      ...
      >
        <template #item.contactName="{ item }">
          <nuxt-link :to="{ name: 'contacts-edit-id', params: { id: item.id } }" class="text-decoration-underline">
            {{ item.contactName }}
          </nuxt-link>
        </template>
        ...
        <template #item.type="{ value }">
          <span>{{ value | mapType(contactTypes) }}</span>
        </template>
        <template #item.actions="{ item }">
          <v-menu offset-y>
            ...
            <v-list>
              <v-list-item :to="{ name: 'contacts-edit-id', params: { id: item.id } }" dense>
                <v-list-item-title>
...
export default defineComponent({
  ...
  setup(props, { root }) {
    ...
    const state = reactive({
      ...
      search: '',
      headers: [
        { text: root.$t('contact.model.name'), value: 'contactName' },
        { text: root.$t('contact.model.businessName'), value: 'businessName' },
        { text: root.$t('contact.model.contactType'), value: 'type' },
        { text: root.$t('common.model.email'), value: 'emailAddress' },
        { text: root.$t('common.model.phone'), value: 'workPhone' },
        { text: root.$t('common.model.mobile'), value: 'mobilePhone' },
        { text: '', value: 'actions', align: 'right', sortable: false }
      ]
    })
    const { loading, conditions, refresh, state: listState } = useGetContacts(state.params)
    ...
    return {
      ...
      loading,
      refresh,
      contacts: toRef(listState, 'items'),
  ...
  async asyncData({ $api }) {
    const { result: contactTypesResult } = await $api.contact.getContactTypes()
