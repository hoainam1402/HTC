<template>
  <v-row dense>
    <v-col cols="12" sm="6">
      <select-contact-type v-model="form.type" :label="$t('contact.model.contactType')" vid="type" rules="required" />
    </v-col>
    <v-col cols="12" sm="6">
      <app-text-field
        v-model="form.contactName"
        :label="$t('contact.model.contactName')"
        vid="contactName"
        :rules="`required|max:30|regex:${regex}`"
      />
    </v-col>
    <v-col cols="12">
      <app-label :label="$t('contact.model.address')" />
      <v-row dense>
        <v-col cols="12" lg="6">
          <app-text-field
            v-model="form.address.street"
            :label="$t('common.model.address')"
            vid="address.street"
            rules="required|max:255"
          />
        </v-col>
        <v-col cols="12" lg="6">
          <app-text-field
            v-model="form.address.ward"
            :label="$t('common.model.ward')"
            vid="address.ward"
            rules="required|max:30"
          />
        </v-col>
        <v-col cols="12" sm="6">
          <select-state
            v-model="form.address.state"
            :label="$t('common.model.state')"
            vid="address.state"
            rules="required"
          />
        </v-col>
        <v-col cols="12" sm="6">
          <app-text-field
            v-model="form.address.postalCode"
            :label="$t('common.model.postalCode')"
            vid="address.postalCode"
            rules="max:30"
          />
        </v-col>
      </v-row>
    </v-col>
    <v-col cols="12">
      <app-label :label="$t('contact.model.contactInformation')" />
      <v-row dense>
        <v-col cols="12" sm="6">
          <app-text-field
            v-model="form.businessName"
            :label="$t('contact.model.businessName')"
            vid="businessName"
            :rules="`required|max:30|regex:${regex}`"
          />
        </v-col>
        <v-col cols="12" sm="6">
          <app-text-field
            v-model="form.emailAddress"
            :label="$t('common.model.email')"
            vid="emailAddress"
            rules="email"
          />
        </v-col>
        <v-col cols="12" sm="6">
          <app-text-field
            v-model="form.workPhone"
            :label="$t('common.model.phone')"
            vid="workPhone"
            rules="phone|max:16"
            onkeypress="return event.charCode >= 48 && event.charCode <= 57"
          />
        </v-col>
        <v-col cols="12" sm="6">
          <app-text-field
            v-model="form.mobilePhone"
            :label="$t('common.model.mobile')"
            vid="mobilePhone"
            rules="phone|max:16"
            onkeypress="return event.charCode >= 48 && event.charCode <= 57"
          />
        </v-col>
      </v-row>
    </v-col>
  </v-row>
</template>

<script>
import { computed, defineComponent, ref } from '@nuxtjs/composition-api'
import AppLabel from '~/components/atoms/global/AppLabel.vue'
import AppTextField from '~/components/atoms/global/AppTextField.vue'
import SelectContactType from '~/components/contact/organisms/SelectContactType.vue'
import SelectState from '~/components/common/molecules/SelectState'
import { REGEX_INPUT } from '@/utils/constants'

export default defineComponent({
  name: 'ContactForm',
  components: {
    AppLabel,
    AppTextField,
    SelectContactType,
    SelectState
  },
  props: {
    value: {
      type: Object,
      default: () => ({})
    }
  },
  setup(props, { emit }) {
    const form = computed({
      get: () => props.value,
      set: (val) => {
        emit('input', val)
      }
    })
    const editMode = computed(() => form.value.id)

    const regex = ref(REGEX_INPUT)

    return {
      form,
      editMode,
      regex
    }
  }
})
</script>
